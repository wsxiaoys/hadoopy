...
cdef __read_key_value_rb(void *fp):
    k = _read_bytes(fp)
    v = _read_bytes(fp)
    return k, v
...
cdef class RawBytesFile(object):
    ...
    def __init__(self, fn=None, mode=None, read_fd=None, write_fd=None, flush_writes=False, unbuffered_reads=False):
        ...
        cdef char *fnc
        cdef char *modec
        self._repr = "RawBytesFile(%s, %s, %s, %s)" % (repr(fn), repr(mode), repr(read_fd), repr(write_fd))
        if fn:
            self.file_method = 'fn'
            if mode == None:
                mode = 'r'
    ...
    def __next__(self):
        if self._read_ptr == <void *>0:
            raise ValueError("Read pointer not set!")
        return __read_key_value_rb(self._read_ptr)

    def write(self, kv):
        if self._write_ptr == <void *>0:
            raise ValueError("Write pointer not set!")
        __write_key_value_rb(self._write_ptr, kv)
        if self.flush_writes:
            self.flush()
